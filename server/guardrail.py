...
# 만 뒤의 숫자는, 그것이 수량/날짜 단위(개·장·종·매·건·번·회·일·월·명·켤레·박스·시)로 이어지지
# 않는 한 만 단위에 합친다. "5만 3천" → "5만 3000" (_CHEON_RE 가 먼저 돎) → 53000 처럼
# 합쳐야 하고, "5만 3개" 는 3이 개수이므로 합치지 않고 "50000 3개" 로 남겨야 한다.
_MAN_RE = re.compile(
    r"(\d[\d,]*)\s*만(?:\s*(\d[\d,]*)(?!\s*(?:개|장|종|매|건|번|회|일|월|명|켤레|박스|시|년|주|분|초)))?"
)
