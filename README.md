...
.venv/bin/python -m eval.eval_router --with-other # OTHER 포함 5클래스 라우팅 153건 (키 필요)
...
**`OTHER` 포함 5클래스 측정 (미분류 카테고리를 별도로 재기)** — 위 대표 지표는 4클래스(`split=="eval"`
120건)만 잰다. 분류체계 밖 문의를 담는 `OTHER` 카테고리 33건(`split=="outscope"`)이 `routing_answers.csv`에
별도로 확보돼 있었지만 대표 지표 어디에도 반영되지 않고 있었다. `eval/eval_router.py --with-other` 옵트인
플래그(기존 4클래스 동작·출력은 그대로 보존)로 그 자리를 채웠다. 재현: `.venv/bin/python -m eval.eval_router
--with-other`.

| 지표 | 4클래스 (기존, n=120) | 5클래스 +OTHER (n=153) |
|---|---|---|
| 정확도 | 0.933 | 0.895 |
| macro F1 | 0.937 | 0.895 |

숫자만 보면 하락처럼 보이지만 **라우터 성능이 나빠진 게 아니다.** 기존 4클래스 측정은 애초에 `OTHER`
33건을 재지 않았고, 이번에 그 33건을 새로 포함시키면서 생긴 차이다 — `OTHER`를 뺀 4클래스만 다시 보면
여전히 0.933/0.937 그대로이고, 4클래스 문의 자체의 분류력은 두 측정 사이에 달라지지 않았다.

다만 `OTHER` 자체는 recall 0.758(25/33)로 나머지 네 라우트의 recall(0.867~0.967)보다 뚜렷이 낮다 —
"별도 확보한 미분류 카테고리가 확보만 해두고 성능이 가장 약한 지점"이라는 것이 이번 측정으로 처음
수치화됐다. 오분류 8건을 직접 읽어 원인을 두 갈래로 나눴다: **(A) 정의 밖 회색지대 5건** — `domain.json`의
`OTHER` 판단 규칙이 매장 위치·타 오픈마켓 주문·제조사 A/S 세 가지만 예시로 들고 있어, 장바구니 소실·재입고
알림·포장 방식 같은 문의가 정의에 없어 가장 가까운 어휘로 끌려간다. **(B) 예약/사전주문 의도 3건** — 정의는
있지만 "주문"이라는 낱말이 직접 들어가 ORDER_PLACE 쪽 어휘 신호가 정의를 압도한다. 자세한 문항별 근거와
확신도 보정표는 `docs/측정기록/2026-09-18 OTHER 포함 5클래스 라우팅 측정.md`에 있다.
...
**도구 호출 적절성을 "정확히 일치"로 엄격화한 뒤의 재측정 (2026-09-18)** — `eval/scoring.py::score_tools`가
기존에는 기대 도구 중 빠진 것(missing)만 보고 초과 호출(extra)은 감점하지 않았는데, 과제 요구사항
원문("실제로 호출한 도구 집합이 기대 도구와 정확히 일치")에 맞춰 **집합 정확 일치**(missing·extra 모두
감점)로 바꿨다. 이 때문에 `eval_answer.py`가 이제 **네 줄**을 찍는다 — 아래는 그 값(잣대가 다른 지표를
같은 줄에 놓지 않도록 라벨을 구분했다):

| 지표 | 값 | 비고 |
|---|---|---|
| 도구 호출 적절성(엄격) | 7/32 (21.9%) | 집합 정확 일치. 새 요구사항 그대로 |
| 답변 적절성 | 24/32 (75.0%) | must/forbid, judge 포함 |
| 종합(엄격) = 위 두 지표 AND | 5/32 (15.6%) | **과거 62.5%·56.2%와 잣대가 달라 직접 비교 불가** |
| 종합(기존 정의, 과거 비교용) | 18/32 (56.3%) | 도구는 missing만 보고 extra는 무시(옛 `score_tools`) — 위 62.5%·56.2%와 같은 잣대 |

엄격 지표(21.9%)가 낮은 주된 이유는 에이전트 오작동이 아니라 **정답셋의 불완전성**이다 —
`answer_goldenset.json` 34건의 `expect.tools`에는 `search_product`가 단 한 번도 없는데, 고객이
"가죽 벨트"처럼 상품명으로 말하면 에이전트는 `search_product`로 이름→product_id를 해석한 뒤에야
`get_product_detail`을 부를 수 있다. 골든셋은 이 해석 단계를 빠뜨리고 최종 조회 도구만 적어 뒀다.
과다호출 27건 중 13건(옛 정의로 재분류하면 통과)이 이 유형이고, 그중 11건은 `search_product` 과다호출
단독 사유다. 실패 사례를 직접 읽어 (A) 골든셋 결함성 과다호출 11건 / (B) 진짜 과다호출 2건 / (C) 필수
도구 미호출 4건 / (D) must 누락 4건 / (E) action 오판 6건(FLAP 2건 포함)으로 나눈 근거와, 행동 판정
혼동표에서 가장 위험한 "기대 ASK→실제 ANSWER" 3건(C-009·C-015·C-027) 해석은
`docs/측정기록/2026-09-18 답변 평가 두 지표 분리 측정.md`에 있다. **골든셋은 이번에 고치지 않았다** —
사실만 기록했다.
...
| # | 바꾼 것 | 라우팅 macro F1 | 답변 통과율 | 회귀 통과 |
|---|---|---|---|---|
| 0 | 기준선 (규칙 라우터) | 0.677 | – | – |
| 1 | LLM 라우터 전환 | 0.945 | 43.8% (14/32, runs=3) | 6/6 PASS ×3회 |
| 2 | 버그 수정·채점기 정규화 | 0.945 | 50.0% (16/32, runs=3, FLAP 2) | 6/6 |
| 3 | A단계: SQLite 실데이터·고객 식별 | 0.945 | (재측정 예정) | 6/6 |
| 4 | 3차 강화 | 0.937 | 규칙 50.0% / judge 포함 62.5% (20/32, runs=3, FLAP 2) | 6/6 ×3 |
| 5 | 진행 상태 정합성 | 0.937 (동일) | 규칙 46.9% / judge 포함 **56.2%**(18/32, runs=3, FLAP 2) — 기준선 62.5% 대비 **하락 6.3%p** | 6/6 (동일) |

각 회차의 자세한 변경 내용과 메모는 아래와 같다.

#### 0 — 기준선 (규칙 라우터)
정확도 0.583.

#### 1 — LLM 라우터 (gpt-4.1-mini) + 답변 gpt-4.1
정확도 0.933, ECE 0.039 · hard 72건 자동처리 0.611, 경계모호 15건 전부 확신 처리(위험)

#### 2 — 검색 동점 버그 수정 · 채점기 만 단위 정규화 · 도구 상한 4 · 주문번호 우선 조회
남은 실패: 되물어야 할 때 답변 3건, must 문자열 의미 불일치 6건(품절/불가/925), 도구 미호출 6건

#### 3 — A단계: SQLite 데이터(상품 180·주문 480) + 발신번호 고객 식별 + 마무리 인사 ASK 오판 수정
eval_context 40건: 자동화율 비회원 0.350 → 식별 고객 0.450, 되묻기율 0.525 → 0.400, 조회율 0.571 → 0.778

#### 4 — 3차 강화: judge 채점기 · 필수 도구 표(부정 결과 → revert) · 확신도 마진 게이트(CONF_MARGIN=0.3) · followup 라우트 이어받기(FOLLOWUP_INHERIT, 기본 꺼짐) · 이관 도구 매핑 · 동의어 보강
judge 판정기 효과 = 바뀌지 않은 에이전트에 judge 만 붙여 규칙 14/32 → judge 포함 21/32(65.6%). 라우트별 필수 도구 표 + 자기 점검 프롬프트는 부정 결과(규칙 13/32·표만 12/32, 기준선 14/32)라 revert. 경계모호 위험 15 → 10건(Task 4 격자 측정 시점 8건)으로 목표 5건 미달, 격자에 5건 이하 조합이 없음. 멀티턴 문맥 주입은 순효과 −1건(턴2+ 정확도 기준선 0.957 → 0.913)이라 이어받기 기본 꺼짐. 측정 원본: docs/측정기록/2026-09-17 3차 강화 측정.md

#### 5 — 진행 상태 정합성: 반품·교환 status_detail 을 반품 단계에서 파생 + 종결 상태(반품완료·교환완료) 신설 · get_order_status 에 active_process 필드 노출 · 답변 규칙 5·6(현재 상태 우선순위, 진행 중 사실 먼저 안내) · 가드레일 VIOLATION_STALE_STATE(SHIPPING·RETURN_REFUND 한정, 재시도해도 안 되면 통화 안 끊음)
실통화 재현 O-1072 4/4 성공(반품 사실 우선 안내). 하락 원인을 가르려 규칙 5·6 만 뺀 **A/B 재측정을 완주**: judge 포함 **53.1%**(17/32, runs=3, FLAP 3) — 규칙을 빼도 회복되지 않음(오히려 −3.1%p, 표본 작아 오차 범위 가능) → **하락 원인은 규칙 5·6 이 아니라 Task 2(get_order_status active_process/events_note 필드)** 로 판정. 근거: C-016(정식 seed 주문, active_process=null)이 규칙 유무와 무관하게 두 실행 모두 동일 사유("7" 누락)로 실패, get_order_status 응답 비대화가 원인으로 추정. C-013·C-021 은 코드 변경으로 설명 안 돼 judge 채점 변동성(FLAP)으로 봄. eval_answer 는 Answerer 직접 호출이라 가드레일을 안 타므로 가드레일 효과는 이 수치에 안 잡힘(재현·회귀로만 확인). 측정 원본: docs/측정기록/2026-09-17 진행상태 정합성 측정.md
